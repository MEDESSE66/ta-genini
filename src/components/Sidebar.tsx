...
import { Archive, Folder, Plus, Settings, Download, Smartphone } from 'lucide-react';
...
export function Sidebar({ isOpen = true, onClose }: SidebarProps) {
  ...
  return (
    <>
      ...
      <aside 
        className={clsx(
            "fixed md:static inset-y-0 left-0 z-50 w-64 bg-zinc-900 border-r border-zinc-800 flex flex-col h-screen text-zinc-400 font-sans shadow-2xl md:shadow-none transition-transform duration-300 ease-in-out",
        ...
        <div className="p-4 border-t border-zinc-800 space-y-1">
          {deferredPrompt && (
            <button 
                ...
                className="flex items-center gap-2 text-sm text-indigo-400 hover:text-indigo-300 transition-colors w-full px-2 py-2 rounded-md hover:bg-zinc-800/30 mb-2"
            >
                <Smartphone size={16} />
          ...
          <div className="px-2 pt-2 text-[10px] text-zinc-700 text-center">
            v1.1 (PWA Ready)
          </div>
        </div>
      </aside>
